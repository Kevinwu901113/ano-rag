...
_RESOURCE_CACHE: Dict[str, Tuple[NoteStore, Indexes]] = {}


def load_resources_once(index_root: Path) -> Tuple[NoteStore, Indexes]:
    key = str(index_root.resolve())
    with _RESOURCE_LOCK:
        cached = _RESOURCE_CACHE.get(key)
        if cached is not None:
            return cached
        note_store = NoteStore(str(index_root / "notes.jsonl"))
        indexes = Indexes(str(index_root / "indexes"))
        _RESOURCE_CACHE[key] = (note_store, indexes)
        return note_store, indexes
...
def _write_notes(units: List[Dict[str, Any]], notes_path: Path, *, overwrite: bool) -> int:
        ...
        for unit in units:
            ...
            note = {
                ...
                "meta": {
                    ...
                    "chunk_idx": unit.get("chunk_idx"),
                    "source": unit.get("mapped_id"),
                    "confidence": 1.0,
                    "final_conf": 1.0,
                    "quality_score": 1.0,
...
def _write_chunks(units: List[Dict[str, Any]], chunks_path: Path, *, overwrite: bool) -> int:
    ...
    with chunks_path.open("w", encoding="utf-8") as handle:
        for unit in units:
            record = {
                ...
                "doc_id": unit.get("doc_name") or "",
                "title": unit.get("doc_name") or "",
                "text": unit.get("text") or "",
                "mapped_id": unit.get("mapped_id"),
                "support": unit.get("support"),
                "meta": {
...
def _ensure_index(
    ...
    notes_written = _write_notes(units, notes_path, overwrite=True)
    chunks_written = _write_chunks(units, chunks_path, overwrite=True)
...
def _prepare_aux_config(index_root: Path, base_cfg: Dict[str, Any]) -> Dict[str, Any]:
    cfg = deepcopy(base_cfg)
    notes_path = index_root / "notes.jsonl"
    indexes_root = index_root / "indexes"
...
def _build_aux_indexes(
    ...
    *,
    ...
    cfg = _prepare_aux_config(index_root, base_cfg)
    notes_path = Path(cfg.get("notes", {}).get("out_path", index_root / "notes.jsonl"))
    stats: Dict[str, Any] = {}
...
def _prepare_retriever_config(index_root: Path, base_cfg: Dict[str, Any], mode: str) -> Dict[str, Any]:
    cfg = _prepare_aux_config(index_root, base_cfg)
    retriever_cfg = cfg.setdefault("retriever", {})
...
def _resolve_chunk_meta_from_ev(ev: Dict[str, Any]) -> Tuple[Optional[str], Optional[str]]:
    chunk_id = ev.get("chunk_id")
    if not chunk_id:
        return None, None
    raw = str(chunk_id)
    if "__" in raw:
        return raw.split("__", 1)[0], raw
    return None, raw
...
def _build_retrieved_context(
    ...
    for rank, ev in enumerate(evidences, start=1):
        ...
        mapped_id = meta.get("mapped_id")
        doc_name = meta.get("doc_name") or ev.get("doc_id")
        support = meta.get("support")
        chunk_id = meta.get("chunk_id") or ev.get("chunk_id") or note_id
        if not mapped_id:
            parsed_mapped, parsed_chunk = _resolve_chunk_meta_from_ev(ev)
            if parsed_mapped:
                mapped_id = parsed_mapped
            if parsed_chunk:
                chunk_id = parsed_chunk
...
def _retrieve_with_backfill(
    *,
    ...
    while True:
        retriever_cfg = _prepare_retriever_config(index_root, base_cfg, mode)
        ...
        retrieve_result = retrieve_answer(
            question=question,
            indexes=indexes,
        ...
        retrieved_context_raw = _build_retrieved_context(evidences, note_store)
...
def _load_entry_config(cfg: Dict[str, Any]) -> Dict[str, Any]:
    entry_cfg = cfg.get("mirage_entry") or cfg.get("entry") or {}
...
def _resolve_retrievers(args: argparse.Namespace, cfg: Dict[str, Any], dataset_cfg: Dict[str, Any]) -> List[str]:
    ...
    *,
    ...
    signature = {
        ...
        "embedding": {
            ...
            "faiss_kind": (embed_cfg.get("faiss") or {}).get("kind"),
        },
        "bm25": {
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run MIRAGE relrag pipeline")
    ...
    parser.add_argument("--retriever", help="Retriever mode: bm25/dense/hybrid")
    parser.add_argument("--reader", help="Reader: vllm/openai")
    parser.add_argument("--cache_dir", help="Cache directory for indexes")
    ...
    modes = _resolve_retrievers(args, cfg, dataset_cfg)
    ...
    index_stats: Dict[str, Any] = {"status": "skipped"}
    if args.force_build or not (prepared_ok and prepared_match):
        ...
        index_stats = _ensure_index(units, cache_root, force_build=True)
        prepared_meta = {
            "cache_key": cache_key,
            "doc_pool_sha1": doc_pool_sha1,
            "doc_pool_path": str(doc_pool_path),
            "doc_chunk_max_chars": int(args.doc_chunk_max_chars or 0),
            "notes_count": index_stats.get("notes"),
            "chunks_count": index_stats.get("chunks"),
    ...
    note_store, indexes = load_resources_once(cache_root)
