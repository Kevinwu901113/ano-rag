...
class MiniGraphRAG:
    ...
    def build_graph(self, passages: List[Dict[str, Any]]):
...
def process_example(
    ...
    *,
    ...
    graph = MiniGraphRAG(encoder, llm)
    graph.build_graph(passages)
    hits = graph.retrieve(question, k=args.topk)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run GraphRAG baseline on MuSiQue")
