...
class VectorRetriever:
    ...
    def search(self, queries: List[str], 
        ...
        # 当向量索引不可用时，尝试BM25/TF-IDF回退
        if self.vector_index.total_vectors == 0:
            if self._bm25_available():
                logger.info("Vector index empty; using BM25/TF-IDF fallback search")
                return self._bm25_search(queries, top_k, similarity_threshold, include_metadata)
            logger.warning("Vector index is empty and no BM25/TF-IDF index available")
            return [[] for _ in queries]
